...
class TERRESTRE_API UTerrainShaper : public UObject
{
	GENERATED_BODY()
public:
	...
	void GenerateTerrainShape(FProtoChunkData& outProtoData,
								FChunkGenerationData& inChunkGenData,
								FVector chunkLocation);
	
	void GenerateChunkInitialData(FChunkGenerationData& outChunkGenData, FVector chunkLocation);
	
	int16 GetTerrainHeightAtXY_Native(double worldX, double worldY, float C, float E, float PV, float W = 0.f);

	float GetSqueezeFactorAtXY_Native(double worldX, double worldY, float C, float E, float PV, float W = 0.f);
	
	UFUNCTION(BlueprintPure, Category = "Terrain Shaper")
	float GetContinentalnessAtXY_BP(double X, double Y);
	UFUNCTION(BlueprintPure, Category = "Terrain Shaper")
	float GetErosionAtXY_BP(double X, double Y);
	UFUNCTION(BlueprintPure, Category = "Terrain Shaper")
	int32 GetTerrainHeightAtXY_BP(double X, double Y);
	UFUNCTION(BlueprintPure, Category = "Terrain Shaper")
	float GetSFAtXY_BP(double X, double Y);
	UFUNCTION(BlueprintPure, Category = "Terrain Shaper")
	float GetInitialDensityAtXYZ_BP(double worldXPos, double worldYPos, double worldZPos);
	UFUNCTION(BlueprintPure, Category = "Terrain Shaper")
	float GetFinalDensityAtXYZ_BP(float initialDensity, double worldZPos, int32 terrainHeight, float SF);
	UFUNCTION(BlueprintPure, Category = "Terrain Shaper")
	EErosionIndex GetErosionIndex_BP(float erosion);
	UFUNCTION(BlueprintPure, Category = "Terrain Shaper")
	EContinentsIndex GetContinentsIndex_BP(float continentalness);
	...
	{
		float densityOffset = DensityNoise1->GetNoise3D(worldXPos, worldYPos, worldZPos) * 3;
		float initialDensity = DensityNoise0->GetNoise3D(densityOffset + worldXPos, 
														 densityOffset + worldYPos, 
														 densityOffset + worldZPos);
	...
	{
		int32 heightDiff = 0 - worldZPos;
		initialDensity *= Settings->FinalDensityMultiplier;
		initialDensity += Settings->FinalDensityShift;
	...
	{
		float output = ContinentalnessNoise0->GetNoise2D(0.25 * X, 0.25 * Y);
		output *= 5.0f;
...
private:
	...
	UPROPERTY()
	TObjectPtr<UTerrainShaperSettings> Settings;
	...
	void SetupNoiseFunctions();

	void DeleteNoiseFunctions();
};
